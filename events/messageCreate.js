...
module.exports = {
  ...
  async execute(message, client) {
    ...
    try {
      console.log(
        `[BOT]: ${message.author.tag} used ${prefix}${commandName} in ${message.guild?.name || "DM"}`,
      );
        ...
        options: {
          getUser: (name) => {
            const mention = args.find(
              (args) => arg.startsWith("<@") && arg.endsWith(">"),
            );
      ...
      console.error(
        `[BOT]: Error executing prefix command ${commandName}: ${error}`,
      );
