...
const event: BotEvent = {
  ...
  execute: async (client: BotClient, interaction: Interaction) => {
    if (interaction.type === InteractionType.ApplicationCommandAutocomplete) {
      const command = client.slashCommands.get(interaction.commandName) as
    ...
    if (interaction.isModalSubmit()) {
      ...
      try {
        ...
        console.error(
          `Modal submit error for customId ${interaction.customId}:`,
          err,
        );
      }
      return;
    ...
    if (interaction.type === InteractionType.ApplicationCommand) {
      ...
      try {
        ...
        if (
          ...
          cooldowns.has(`${interaction.user.id}|${command.name}`)
        ) {
          ...
          if (
            ...
            (!interaction.memberPermissions ||
              !interaction.memberPermissions.has(
                PermissionsBitField.resolve(slashCmd.user_perms),
              ))
          ) {
            await interaction.reply({
          ...
          if (
            ...
            (!botMember ||
              !botMember.permissions.has(
                PermissionsBitField.resolve(slashCmd.bot_perms),
              ))
          ) {
            await interaction.reply({
              content: `The bot doesn't have the required permissions to run this command.`,
              ephemeral: true,
            });
            return;
          }
        ...
        if (commandWithCooldown.cooldown) {
          cooldowns.set(
            ...
            Date.now() + commandWithCooldown.cooldown,
          );
          setTimeout(() => {
        ...
        console.error(err);
        const payload = {
          content: `The bot ran into an error executing that command.`,
          ephemeral: true,
        };
        ...
        console.log(
          `Failed to execute command /${command.name} --> ${err.message}`,
        );
      }
    }
  },
