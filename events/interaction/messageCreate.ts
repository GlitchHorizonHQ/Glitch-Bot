...
const event: BotEvent = {
  ...
  execute: async (client: BotClient, message: Message): Promise<void> => {
    ...
    msg.return = async (
      text: string,
      color: ColorResolvable = client.settings.color,
    ) => {
    ...
    if (command) {
      if (
        command.cooldown &&
        cooldowns.has(`${message.author.id} | ${command.name}`)
      ) {
        msg.error(
          `You are on a **${ms(cooldowns.get(`${message.author.id} | ${command.name}`)! - Date.now(), { long: true })}** cooldown.`,
        );
        return;
      }
      ...
      if (command.user_perms || command.bot_perms) {
        if (
          ...
          !message.member?.permissions.has(
            PermissionsBitField.resolve(command.user_perms),
          )
        ) {
          msg.error(
        ...
        if (
          ...
          (!botMember ||
            !botMember.permissions.has(
              PermissionsBitField.resolve(command.bot_perms),
            ))
        ) {
          msg.error(
      ...
      try {
        ...
        console.log(err);
        msg.error(`The bot ran into an error excuting that command.`);
      }
